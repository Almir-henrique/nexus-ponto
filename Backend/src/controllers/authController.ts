...
export async function loginFuncionario(
  ...
  if (
    ...
    return res.status(400).json({
      mensagem:
        "Código da empresa, matrícula e senha são obrigatórios.",
    });
  }
  ...
  try {
    ...
    if (!funcionario) {
      return res.status(401).json({
        mensagem:
          "Código da empresa, matrícula ou senha incorretos.",
      });
    }
